...
***** Ostensibly needs ICD age/gender splits--see SAS code for details

   * Keep only valid face-to-face claims; 
   *proc sql;
   *create table out.cc_long_&claimtype._&year. as
   *select distinct a.enrolid, a.DX_ICD
   *from out.cc_long_&claimtype._&year. as a
   *left join in.ms_f_&year. as c
   *   on a.FACHDID = c.FACHDID
   *where substr(c.BILLTYP,1,2) = ) ///11) ///
   *   or put(substr(a.proc1,1,5),$ValidProc.)=) ///Yes) ///
*	;
   *quit;
** Keeping only valid face to face claims
* Note: can add here details from Tzu-Chun) ///s valid proc codes excel sheet

**** For use on the SCC *****
cd /project/marketscan/
use Claims_to_HCC_Construction.dta, clear
******************************

// gen dxver = .   
// gen newdate = date(svcdate, "MDY") 
// replace newdate = date(admdate, "MDY") if missing(newdate)
// drop if missing(newdate)
// replace dxver = 9 if (newdate < date("10/01/2015", "MDY"))
// replace dxver = 0 if missing(dxver)

// keep enrolid year dx* // newdate dxver
// drop if missing(dx1) & missing(dx2) & missing(dx3) & missing(dx4)

***** ICD-9
*** Note: I want to use the labelling procedure for the dx, which means a numeric variable
*** The only strings in dx* for ICD-9 are V and E, so I replace E with 88888 and V with 99999. 
foreach var of varlist dx1 dx2 dx3 dx4 { 
	replace `var' = regexr(`var',"E","88888")
	replace `var' = regexr(`var',"V","99999")
	} 

destring dx1, gen(hcc1_1)
destring dx2, gen(hcc1_2)
destring dx3, gen(hcc1_3)
destring dx4, gen(hcc1_4)
order hcc1_*, last

** This maps from dx codes to hcc1 (when dxver = 9) 
recode hcc1_1-hcc1_4 /// 
    (0031 = 2) ///
    (00321 = 3) ///
    (00323 = 55) ///
    (00324 = 55) ///
...
** For HCC2
destring dx1, gen(hcc2_1)
destring dx2, gen(hcc2_2)
destring dx3, gen(hcc2_3)
destring dx4, gen(hcc2_4)
order hcc2_*, last

recode hcc2_1-hcc2_4 ///
    (40403 = 187) ///
    (40413 = 187) ///
    (40493 = 187) ///
    (74100 = 121) ///
...
drop if missing(hcc1_1) & missing(hcc1_2) & missing(hcc1_3) & missing(hcc1_4) & ///
		missing(hcc2_1) & missing(hcc2_2) & missing(hcc2_3) & missing(hcc2_4)
duplicates drop

egen test = concat(hcc1_1 hcc1_2 hcc1_3 hcc1_4 hcc2_1 hcc2_2 hcc2_3 hcc2_4), punct(,)
sort enrolid hcc1_1
bysort enrolid: replace test = test[_n-1] + "," + test if _n > 1
collapse (last) test, by(enrolid year)
split test, parse(,)
drop test
...
label values test* cc_label
egen num_hccs = rownonmiss(test*)
save "AllEnrollees_HCC_Count.dta", replace

*** ICD-10 mapping not needed yet. 
