...
#
# PURPOSE: This file predicts pay_sim and sample_sstar for the full sample based on equilibrium parameters 
# 		which may have been estimated on a subsample
#
# NOTES: 
#
################################################################################
...
belief_pi2 <- .035
belief_pi3 <- 0.025
rowguess <- Inf # For exporting to the outvec
...
# For now, randomly sample 10,000 households (just to get a handle on timing)
...
samplefams <- c(sample(all_treated_fams, 500), sample(all_control_fams, 500))
...
### Solving the model: put this into an iterative function for contraction mapping
  ...
  SMData_20210301 <- SMData_20210301 %>% ungroup() %>% 
   ...
   save.image("/project/caretaking/Outputs/StructuralModel/Updated_20221201/EquilibriumData_20221206.RData")

   # Expected utility in each period
   SMData_20210301$exp_u <- 1:nrow(SMData_20210301) %>% future_map_dbl(function(x) integrate_eui(x,df=SMData_20210301))
   SMData_20210301 <- SMData_20210301 %>% group_by(enrolid) %>%
           mutate(V_i = ifelse(!is.na(lead(exp_u,n=7)),exp_u + delta*lead(exp_u,1) + delta^2*lead(exp_u,2)+ delta^3*lead(exp_u,3)+ delta^4*lead(exp_u,4)+ delta^5*lead(exp_u,5)+ delta^6*lead(exp_u,6)+ delta^7*lead(exp_u,7), NA)) %>%
           mutate(V_i = ifelse(!is.na(lead(exp_u,n=6)),exp_u + delta*lead(exp_u,1) + delta^2*lead(exp_u,2)+ delta^3*lead(exp_u,3)+ delta^4*lead(exp_u,4)+ delta^5*lead(exp_u,5)+ delta^6*lead(exp_u,6), NA)) %>%
           mutate(V_i = ifelse(!is.na(lead(exp_u,n=5)),exp_u + delta*lead(exp_u,1) + delta^2*lead(exp_u,2)+ delta^3*lead(exp_u,3)+ delta^4*lead(exp_u,4)+ delta^5*lead(exp_u,5), NA)) %>%
           mutate(V_i = ifelse(!is.na(lead(exp_u,n=4)),exp_u + delta*lead(exp_u,1) + delta^2*lead(exp_u,2)+ delta^3*lead(exp_u,3)+ delta^4*lead(exp_u,4), NA)) %>%
           mutate(V_i = ifelse(!is.na(lead(exp_u,n=3)),exp_u + delta*lead(exp_u,1) + delta^2*lead(exp_u,2)+ delta^3*lead(exp_u,3), NA)) %>%
           mutate(V_i = ifelse(!is.na(lead(exp_u,n=2)),exp_u + delta*lead(exp_u,1) + delta^2*lead(exp_u,2), NA)) %>%
           mutate(V_i = ifelse(!is.na(lead(exp_u,n=1)),exp_u + delta*lead(exp_u,1), exp_u)) 
