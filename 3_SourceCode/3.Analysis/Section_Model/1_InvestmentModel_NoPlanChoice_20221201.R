...
belief_pi1 <- runif(n=1,min=-.1,max=.2) # starting guess, effect of others' chronic illness (in percent)
...
# p1 <- seq(from=0,to=.15,by=.05)
# p2 <- seq(from=0,to=2,by=1)
# p3 <- seq(from=0,to=1,by=.5)
# p4 <- seq(from=0,to=.15,by=.05)
# p5 <- seq(from=0,to=.15,by=.05)
# p6 <- seq(from=0,to=.15,by=.05)
...
if (!file.exists("/project/caretaking/Outputs/StructuralModel/Updated_20221201/ModelData_20221128.RData")) {
  ...
  # Merge in calibrated data
  load(here("2_Data/7.StructuralModel/","MainData.RData"))
  tomerge <- mydata %>% select(c("famid","year","omega")) %>% 
    group_by(famid,year) %>% summarize(omega = mean(omega))
  SMData_20210301 <- SMData_20210301 %>% 
    left_join(tomerge,by=c("famid","year"),all.x=T,all.y=F) # Currently at the family level, will need to update
  # Calibrated omega
  SMData_20210301 <- SMData_20210301 %>% filter(!is.na(omega)) # eventually, remove this
  
  SMData_20210301$beta <- .9 # Calibrated beta
  
  # Merge in distribution of labmda_TR
  indshocks <- read.dta(here("2_Data/CodeCleaning_Data/Model/","IndividualShocks.dta"))
  
  # Construct the three reference points (mean +/- SD) for integrating EU
  indshocks <- indshocks %>% mutate(spend_1 = exp(mymean - mysd)*52,
                                    ...
                                    prob_3 = pshifted_lnorm(mymean+mysd,meanlog=mymean,sdlog=mysd,shift=myshift)-
                                                pshifted_lnorm(mymean+mysd-1,meanlog=mymean,sdlog=mysd,shift=myshift))
  indshocks <- indshocks %>% mutate(totprob = prob_1 + prob_2 + prob_3) %>%
    mutate(prob_1 = prob_1/totprob,
           prob_2 = prob_2/totprob,
           prob_3 = prob_3/totprob) %>% select(-c(totprob))
  
  # Merge these to individuals in data
  indshocks$test <- "A"
  indshocks <- indshocks %>% rowwise() %>% 
    mutate(enrolid2 = paste0(test,as.character(enrolid),sep="")) %>% 
    select(-c("enrolid","test")) %>% 
    rename(enrolid=enrolid2)
  
  SMData_20210301 <- SMData_20210301 %>% left_join(indshocks,by=c("enrolid","famid","year"),all.x=T,all.y=F)
  SMData_20210301 <- SMData_20210301 %>% filter(!is.na(mymean)) # TODO: remove this
  ...
  rm(tomerge,allmch,indshocks)
  gc()
  
  # Merge in simulated shock based on SEED above
  load(file = paste0("C:/Users/alexh/Dropbox/IBNR_MoralHazard_Github/2_Data/Model/Shock_",myseed,".Rdata",sep="")) # Individual shock to merge
  shock <- structure(shock, row.names = c(NA,-1), class = "data.frame")
  mydata <- cbind(mydata,transpose(shock))
  names(mydata)[12] <- c("healthshock")
  tomerge <- mydata %>% select(c("famid","year","healthshock")) %>% # Currently at the family level, will need to update 
    group_by(famid,year) %>% summarize(healthshock = sum(healthshock))
  SMData_20210301 <- SMData_20210301 %>%
    left_join(tomerge,by=c("famid","year"),all.x=T,all.y=F)
  SMData_20210301 <- SMData_20210301 %>% filter(!is.na(healthshock))
  
  rm(shock,mydata,tomerge)
  gc()
  ...
  save(SMData_20210301,file=here("2_Data/CodeCleaning_Data","ModelData_20221128.RData"))
}
...
# Some data prep steps -- move these into loop in next iteration: 
# Calculate indicator for relative T (at enrollee level)
SMData_20210301 <- SMData_20210301 %>% group_by(enrolid) %>% 
  mutate(nyear = year - min(year)+1)

# Drop enrollees only observed in one year 
SMData_20210301 <- SMData_20210301 %>% group_by(enrolid) %>% 
...
##### 2. Main model: calculate prediction error from guess ##### 
  # Average (annual) OOP cost of preventive care
...
### Solving the model: put this into an iterative function for contraction mapping
  ...
  moment_prevRMSPE <- SMData_20210301 %>% mutate(diff = (newprev_numvisits - sample_sstar)^2) %>%
    ungroup() %>% summarize(diff = sqrt(mean(diff,na.rm=T))) %>% as.numeric()
  ...
  moment_prevMean <- abs(mean(SMData_20210301$newprev_numvisits)-mean(SMData_20210301$sample_sstar,na.rm=T))
  moment_prevMed <- abs(median(SMData_20210301$newprev_numvisits)-median(SMData_20210301$sample_sstar,na.rm=T))
  ...
  coef_pop_prev <- fepois(newprev_numvisits~treated_post | famid + year, data=SMData_20210301 %>% filter(todrop_c == 0))$coefficients[1]
  ...
  # Scale by SD of tot_pay and newprev_numvisits
  ...
  moment_prevMean <- moment_prevMean/sd(SMData_20210301$newprev_numvisits, na.rm=T)
  moment_prevMed <- moment_prevMed/sd(SMData_20210301$newprev_numvisits, na.rm=T)
  moment_prevRMSPE <- moment_prevRMSPE/sd(SMData_20210301$newprev_numvisits, na.rm=T)
  
  # Scale DD coefficients by SD of asinh(tot_pay) and log(numvisits+1)
  moment_payDD <- moment_payDD / sd(asinh(SMData_20210301$newprev_numvisits),na.rm=T)
  moment_prevDD <- moment_prevDD / sd(log(SMData_20210301$newprev_numvisits+1),na.rm=T)
  moment_payMean
  moment_payMed
  moment_payRMSPE
  moment_payDD
  ...
  outvec <- data.frame(t(c(starting_p, p_prior_var, prev_post_var, belief_pi1, belief_pi2, belief_pi3, loss)))
  write.table(outvec, file = "/project/caretaking/Outputs/StructuralModel/Updated_20221201/LossOutputs.csv", append = TRUE,
              quote = TRUE, sep=" , ", row.names = FALSE, col.names = FALSE)
  outvec 

save.image(paste0("/project/caretaking/Outputs/StructuralModel/Updated_20221201/Working_",seed,".RData",sep=""))
