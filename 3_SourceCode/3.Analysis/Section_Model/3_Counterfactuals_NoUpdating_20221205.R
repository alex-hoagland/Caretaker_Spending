...
#
# PURPOSE: This file predicts counterfactual utility in the absence of belief updating after MHE
#
# NOTES: 
#
################################################################################
...
belief_pi2 <- .035
belief_pi3 <- 0.025
rowguess <- Inf # For exporting row 
...
post_mean <- function(df,j) { # Function of number of signals and mean of signals (p-guess)
  ...
  if(df[j,]$numsignals == 0) { 
    ...
    sp <- log((starting_p)/(1-(starting_p)))
  ...
  lstart <- log((starting_p)/(1-starting_p)) # Include discrete shift here
...
# For now, randomly sample 2,000 households (just to get a handle on timing)
SMData_20210301 <- SMData_20210301 %>% filter(famid %in% sample(famid, 2000))
...
save.image("/project/caretaking/Outputs/StructuralModel/Updated_20221201/Counterfactual_NoUpdating_20221206.RData")
