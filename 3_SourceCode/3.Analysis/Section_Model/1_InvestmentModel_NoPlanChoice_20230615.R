...
#
# NOTES:
# todo: update data based on full distributions of shocks, etc.
# todo: parallelize?
# updated 6/16/2023: new parameters for flexibility.

# todo: add in (a) link between preventive care and costs in two ways (simple and complex), 
# (b) unequal penalties for disutility, (c) arbitrary correlation between TR and CH
################################################################################
...
### Starting guesses
# Guesses include: beliefs (prior mean, prior variance)
#                  belief updating (regression coefficients, variance)
#                  predicted investments in wellness visits

# # Intervals to draw starting guesses from 
#starting_p <- runif(n=1,min=0,max=.15) # Start in percentages (2%)
#p_prior_var <- runif(n=1,min=0,max=2.5) # starting variance
#prev_post_var <- runif(n=1,min=0,max=1) # variance of preventive services (beta, multiplied by preventive spending)

#belief_pi1 <- runif(n=1,min=-.1,max=.2) # starting guess, the effect of others' chronic illness (in percent)
#belief_pi2 <- runif(n=1,min=-.1,max=.2) # starting guess, effect of others' acute illness (in percent)
#belief_pi3 <- runif(n=1,min=-.1,max=.2) # starting guess, effect of own acute illness (in percent)
#     # DROPPED FOR NOW # belief_pi4 <- -.01 # starting guess, years since event (in percent)
#                                           
# # Or structured guesses
# p1 <- seq(from=0.005,to=.05,by=.005)
# p2 <- seq(from=2.5,to=2.75,by=.05)
# p3 <- seq(from=1.1,to=1.2,by=.01)
# p4 <- seq(from=0.08,to=.12,by=.001)
# p5 <- seq(from=0.02,to=.04,by=.001)
# p6 <- seq(from=0,to=.02,by=.001)
# myguesses <- expand.grid(p1,p2,p3,p4,p5,p6)
# names(myguesses) <- c("starting_p","p_prior_var","prev_post_var",
#                       "belief_pi1","belief_pi2","belief_pi3")

#rowguess <- sample(seq(from=1,to=nrow(myguesses),by=1),1)
#starting_p <- myguesses[rowguess ,1]
#p_prior_var <- myguesses[rowguess ,2]
#prev_post_var <- myguesses[rowguess ,3]
#belief_pi1 <- myguesses[rowguess ,4]
#belief_pi2 <- myguesses[rowguess ,5]
#belief_pi3 <- myguesses[rowguess ,6]

starting_p <- runif(n=1,min=.005,max=.03) 
p_prior_var <- runif(n=1,min=2,max=2.5) 
prev_post_var <- runif(n=1,min=.5,max=1.5) 
belief_pi1 <- runif(n=1,min=.08,max=.12) 
belief_pi2 <- runif(n=1,min=.02,max=.04) 
belief_pi3 <- runif(n=1,min=0,max=.04) 
rowguess <- seed # For exporting to the outvec

# Starting guesses
# starting_p <- .02 # Start in percentages (2%)
# p_prior_var <- 1 # starting variance
# prev_post_var <- .2 # variance of preventive services (beta, multiplied by preventive spending)
# 
# belief_pi1 <- .05 # starting guess, effect of others' chronic illness (in percent)
# belief_pi2 <- .05 # starting guess, effect of others' acute illness (in percent)
# belief_pi3 <- .05 # starting guess, effect of own acute illness (in percent)
# DROPPED FOR NOW # belief_pi4 <- -.01 # starting guess, years since event (in percent)

# Global model parameters
delta <- .95 # Annual discount rate of .95 (not a guess)
alpha <- 1 # disutility from under-shooting care -- note base case is alpha = 1
gamma <- 1 # effect on future health needs based on preventive care. Note base case is gamma = 1
g1 <- 1 # Additional gamma parameters (not used yet)
g2 <- 1 # Additional gamma parameters (not used yet)
rho <- 0 # arbitrary correlation between lambda_TR and lambda_CH. Base case is rho = 0. 
################################################################################
...
if (!file.exists("2_Data/CodeCleaning_Data/ModelData_20221201.RData")) {
  ...
  # Health shocks are affected by preventive care in the previous period (?) 
...
load(file="2_Data/CodeCleaning_Data/ModelData_20221201.RData")
...
set.seed(123) # need to draw same families every time
...
##### 2. Main model: calculate prediction error from guess ##### 
  ...
  load(file="2_Data/CodeCleaning_Data/ModelData_20221128.RData")
...
### Solving the model: put this into an iterative function for contraction mapping
  ...
  ### THOSE OBSERVED FOR 2 YEARS ####
      ...
      allouts <- foreach (i = 1:nrow(allcomb)) %dopar% { # Calculate V1,V2,V3 and total V for each combination
        ...
        tomerge <- allcomb[i,] %>% select(-c("num")) %>% 
          ...
          mutate(nyear = as.numeric(nyear), 
                 healthshock_correction = ifelse(lag(value) > 0, gamma, 1))
        tomerge$healthshock_correction[is.na(tomerge$healthshock_correction)] <- 1
        ...
        sub_2$healthshock <- ifelse(sub_2$healthshock > 0, sub_2$healthshock * sub_2$healthshock_correction, 
                                    sub_2$healthshock * (1+ sub_2$healthshock_correction))
          # Correct health shocks if needed (make negative health shocks smaller)
      ...
      myopts$sample_sstar <- seq_len(nrow(myopts)) %>% 
  ...
  #### FOR INDIVIDUALS OBSERVED FOR 3 YEARS ####
      ...
      allouts <- foreach (i = 1:nrow(allcomb)) %dopar% { # Calculate V1,V2,V3 and total V for each combination
        ...
        sub_3 <- sub_3 %>% left_join(tomerge,by=c("nyear"),all.x=T,all.y=F) %>% 
          mutate(numsignals = value)  %>% select(-c("value"))
        sub_3$healthshock <- ifelse(sub_3$healthshock > 0, sub_3$healthshock * sub_3$healthshock_correction, 
                          sub_3$healthshock * (1+ sub_3$healthshock_correction))
  ...
  #### FOR INDIVIDUALS OBSERVED FOR 4 YEARS ####
      ...
      # pb = txtProgressBar(min = 1, max = nrow(allcomb), initial = 1) 
      allouts <- foreach (i = 1:nrow(allcomb)) %dopar% { 
        # Calculate V1,V2,V3 and total V for each combination
        # setTxtProgressBar(pb,i)
        ...
        sub_4 <- sub_4 %>% left_join(tomerge,by=c("nyear"),all.x=T,all.y=F) %>% 
          mutate(numsignals = value)  %>% select(-c("value"))
        sub_4$healthshock <- ifelse(sub_4$healthshock > 0, sub_4$healthshock * sub_4$healthshock_correction, 
                          sub_4$healthshock * (1+ sub_4$healthshock_correction))
  ...
  #### FOR INDIVIDUALS OBSERVED FOR 5 YEARS ####
      ...
      allouts <- foreach (i = 1:nrow(allcomb)) %dopar% { # Calculate V1,V2,V3 and total V for each combination
        ...
        sub_5 <- sub_5 %>% left_join(tomerge,by=c("nyear"),all.x=T,all.y=F) %>%
          mutate(numsignals = value)  %>% select(-c("value"))
        sub_5$healthshock <- ifelse(sub_5$healthshock > 0, sub_5$healthshock * sub_5$healthshock_correction, 
                          sub_5$healthshock * (1+ sub_5$healthshock_correction))
    ...
    ################
      ...
      #     mutate(numsignals = value)  %>% select(-c("value"))
        # sub_6$healthshock <- ifelse(sub_6$healthshock > 0, sub_6$healthshock * sub_6$healthshock_correction, 
        #                             sub_6$healthshock * (1+ sub_6$healthshock_correction))
      ...
      #     mutate(numsignals = value)  %>% select(-c("value"))
        # sub_7$healthshock <- ifelse(sub_7$healthshock > 0, sub_7$healthshock * sub_7$healthshock_correction, 
        #                             sub_7$healthshock * (1+ sub_7$healthshock_correction))
      ...
      # #     mutate(numsignals = value)  %>% select(-c("value"))
      #   sub_8$healthshock <- ifelse(sub_8$healthshock > 0, sub_8$healthshock * sub_8$healthshock_correction, 
      #                               sub_8$healthshock * (1+ sub_8$healthshock_correction))
      ...
      # myopts$sample_sstar <- seq_len(nrow(myopts)) %>%
      #   future_map_dbl(function(x) {
      #     myout <- allcomb %>% filter(num == myopts[x,]$opts) # nolint: Grammarly.
      #     if (nrow(myout) == 0) { return(NA) }
      #     c <- myopts[x,]$nyear
      #     return(as.numeric(myout[,c]))
      #   })
  ...
  # If these regressions don't work, then prediction is very bad # nolint: Grammarly.
  ...
  # For now, uniform weights, so just sum moments
  loss <- moment_payMean + moment_payMed + moment_payRMSPE + moment_payDD + 
    moment_prevMean + moment_prevMed + moment_prevRMSPE + moment_prevDD
  loss <- as.numeric(loss)
  loss
  
  ## STEP 5: Run this code multiple times for different starting guesses
  # If running individual processes, then save image and append parameters + loss to external table?
  outvec <- data.frame(t(c(rowguess, starting_p, p_prior_var, prev_post_var, belief_pi1, belief_pi2, belief_pi3, loss)))
  write.table(outvec, file = "/project/caretaking/Outputs/StructuralModel/Updated_20230616/LossOutputs.csv", append = TRUE,
              quote = TRUE, sep=" , ", row.names = FALSE, col.names = FALSE)
  outvec 

save.image(paste0("/project/caretaking/Outputs/StructuralModel/Updated_20230616/Working_",seed,".RData",sep=""))
