...
proc sql; 
   create table out.allenrollees_hcc13_2015 as 
   select enrolid from out.allclaims_hcc13_2015
   group by enrolid; 
quit; 

data out.allenrollees_hcc13_2015;
   set out.allenrollees_hcc13_2015;

   famid = floor(enrolid/100); 
run; 

/* --- 1. Enrollment Info for all involved families -----------------------------------------*/; 
data out.allenrollment_13_2015; 
   if _N_=1 then do;
   declare hash ids(dataset:"out.allenrollees_hcc13_2015");
   ids.definekey('famid');
   ids.definedone();
   end;
