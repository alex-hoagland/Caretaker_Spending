...
proc sql; 
   create table out.allenrollees_hcc56_2018 as 
   select enrolid from out.allclaims_hcc56_2018
   group by enrolid; 
quit; 

data out.allenrollees_hcc56_2018;
   set out.allenrollees_hcc56_2018;

   famid = floor(enrolid/100); 
run; 

/* --- 1. Enrollment Info for all involved families -----------------------------------------*/; 
data out.allenrollment_56_2018; 
   if _N_=1 then do;
   declare hash ids(dataset:"out.allenrollees_hcc56_2018");
   ids.definekey('famid');
   ids.definedone();
   end;
