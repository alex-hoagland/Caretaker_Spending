...
/*------------------------------------------------------------------------*
 * 		ORDER OF OPERATIONS					  *
 * 0. Identify all with HCC_21 claims in 2006-2018			  *
 * 1. Pull enrollment files for all families in (0.)			  *
 * 2. The rest is done in stata						  *
 *------------------------------------------------------------------------*/;

/* --- 1. Enrollment Info for all involved families -----------------------------------------*/; 
proc import datafile="/project/caretaking/IdentifyPHat/allenrollees_hcc21.dta"
   out = out.allenrollees_hcc21
   dbms = stata
   replace;
run;

data out.allenrollment_21; 
   if _N_=1 then do;
   declare hash ids(dataset:"out.allenrollees_hcc21");
   ids.definekey('famid');
   ids.definedone();
   end;
