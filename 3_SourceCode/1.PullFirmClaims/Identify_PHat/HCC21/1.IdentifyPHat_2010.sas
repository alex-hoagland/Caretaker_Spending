...
proc sql; 
   create table out.allenrollees_hcc21_2010 as 
   select enrolid from out.allclaims_hcc21_2010
   group by enrolid; 
quit; 

data out.allenrollees_hcc21_2010;
   set out.allenrollees_hcc21_2010;

   famid = floor(enrolid/100); 
run; 

/* --- 1. Enrollment Info for all involved families -----------------------------------------*/; 
data out.allenrollment_21_2010; 
   if _N_=1 then do;
   declare hash ids(dataset:"out.allenrollees_hcc21_2010");
   ids.definekey('famid');
   ids.definedone();
   end;
