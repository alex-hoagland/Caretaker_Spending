...
proc sql; 
   create table out.allenrollees_hcc21_2008 as 
   select enrolid from out.allclaims_hcc21_2008
   group by enrolid; 
quit; 

data out.allenrollees_hcc21_2008;
   set out.allenrollees_hcc21_2008;

   famid = floor(enrolid/100); 
run; 

/* --- 1. Enrollment Info for all involved families -----------------------------------------*/; 
data out.allenrollment_21_2008; 
   if _N_=1 then do;
   declare hash ids(dataset:"out.allenrollees_hcc21_2008");
   ids.definekey('famid');
   ids.definedone();
   end;
