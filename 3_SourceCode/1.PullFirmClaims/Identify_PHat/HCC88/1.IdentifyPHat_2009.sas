...
proc sql; 
   create table out.allenrollees_hcc88_2009 as 
   select enrolid from out.allclaims_hcc88_2009
   group by enrolid; 
quit; 

data out.allenrollees_hcc88_2009;
   set out.allenrollees_hcc88_2009;

   famid = floor(enrolid/100); 
run; 

/* --- 1. Enrollment Info for all involved families -----------------------------------------*/; 
data out.allenrollment_88_2009; 
   if _N_=1 then do;
   declare hash ids(dataset:"out.allenrollees_hcc88_2009");
   ids.definekey('famid');
   ids.definedone();
   end;
