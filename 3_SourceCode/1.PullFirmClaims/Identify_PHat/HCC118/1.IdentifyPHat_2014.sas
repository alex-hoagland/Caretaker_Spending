...
data out.allclaims_hcc118_2014; 
...
proc sql; 
   create table out.allenrollees_hcc118_2014 as 
   select enrolid from out.allclaims_hcc118_2014
   group by enrolid; 
quit; 

data out.allenrollees_hcc118_2014;
   set out.allenrollees_hcc118_2014;

   famid = floor(enrolid/100); 
run; 

/* --- 1. Enrollment Info for all involved families -----------------------------------------*/; 
data out.allenrollment_118_2014; 
   if _N_=1 then do;
   declare hash ids(dataset:"out.allenrollees_hcc118_2014");
   ids.definekey('famid');
   ids.definedone();
   end;
