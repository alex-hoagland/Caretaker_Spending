...
proc sql; 
   create table out.allenrollees_hcc48_2011 as 
   select enrolid from out.allclaims_hcc48_2011
   group by enrolid; 
quit; 

data out.allenrollees_hcc48_2011;
   set out.allenrollees_hcc48_2011;

   famid = floor(enrolid/100); 
run; 

/* --- 1. Enrollment Info for all involved families -----------------------------------------*/; 
data out.allenrollment_48_2011; 
   if _N_=1 then do;
   declare hash ids(dataset:"out.allenrollees_hcc48_2011");
   ids.definekey('famid');
   ids.definedone();
   end;
