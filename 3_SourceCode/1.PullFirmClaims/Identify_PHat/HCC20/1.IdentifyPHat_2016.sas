...
proc sql; 
   create table out.allenrollees_hcc20_2016 as 
   select enrolid from out.allclaims_hcc20_2016
   group by enrolid; 
quit; 

data out.allenrollees_hcc20_2016;
   set out.allenrollees_hcc20_2016;

   famid = floor(enrolid/100); 
run; 

/* --- 1. Enrollment Info for all involved families -----------------------------------------*/; 
data out.allenrollment_20_2016; 
   if _N_=1 then do;
   declare hash ids(dataset:"out.allenrollees_hcc20_2016");
   ids.definekey('famid');
   ids.definedone();
   end;
