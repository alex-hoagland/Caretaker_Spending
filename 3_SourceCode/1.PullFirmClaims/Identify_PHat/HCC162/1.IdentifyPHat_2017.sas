...
data out.allclaims_hcc162_2017; 
...
proc sql; 
   create table out.allenrollees_hcc162_2017 as 
   select enrolid from out.allclaims_hcc162_2017
   group by enrolid; 
quit; 

data out.allenrollees_hcc162_2017;
   set out.allenrollees_hcc162_2017;

   famid = floor(enrolid/100); 
run; 

/* --- 1. Enrollment Info for all involved families -----------------------------------------*/; 
data out.allenrollment_162_2017; 
   if _N_=1 then do;
   declare hash ids(dataset:"out.allenrollees_hcc162_2017");
   ids.definekey('famid');
   ids.definedone();
   end;
