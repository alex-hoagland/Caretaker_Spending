...
proc sql; 
   create table out.allenrollees_hcc12_2006 as 
   select enrolid from out.allclaims_hcc12_2006
   group by enrolid; 
quit; 

data out.allenrollees_hcc12_2006;
   set out.allenrollees_hcc12_2006;

   famid = floor(enrolid/100); 
run; 

/* --- 1. Enrollment Info for all involved families -----------------------------------------*/; 
data out.allenrollment_12_2006; 
   if _N_=1 then do;
   declare hash ids(dataset:"out.allenrollees_hcc12_2006");
   ids.definekey('famid');
   ids.definedone();
   end;
