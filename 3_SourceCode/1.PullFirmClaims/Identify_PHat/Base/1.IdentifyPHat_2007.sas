...
data out.allclaims_hccHHH_2007; 
...
proc sql; 
   create table out.allenrollees_hccHHH_2007 as 
   select enrolid from out.allclaims_hccHHH_2007
   group by enrolid; 
quit; 

data out.allenrollees_hccHHH_2007;
   set out.allenrollees_hccHHH_2007;

   famid = floor(enrolid/100); 
run; 

/* --- 1. Enrollment Info for all involved families -----------------------------------------*/; 
data out.allenrollment_HHH_2007; 
   if _N_=1 then do;
   declare hash ids(dataset:"out.allenrollees_hccHHH_2007");
   ids.definekey('famid');
   ids.definedone();
   end;
