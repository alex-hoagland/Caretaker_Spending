...
/*------------------------------------------------------------------------*
 * 		ORDER OF OPERATIONS					  *
 * 0. Identify all HCC_12 claims in 2006				  *
 * 1. Identify all *new* HCC_12 claims in (2007-2010)			  *
...
data out.pe_12; 
   ...
   if dx1 in: ('C4A0', 'C4A10', 'C4A11', 'C4A12', 'C4A20', 'C4A21', 'C4A22', 'C4A30', 'C4A31', 'C4A39', 'C4A4', 'C4A51', 'C4A52', 'C4A59', 'C4A60', 'C4A61', 'C4A62',
		...
		'2273', '2274', '22802', '2370', '2371', '2373', '2375', '2376', '2379', '2396', '7595', '7596') or 
      dx2 in: ('C4A0', 'C4A10', 'C4A11', 'C4A12', 'C4A20', 'C4A21', 'C4A22', 'C4A30', 'C4A31', 'C4A39', 'C4A4', 'C4A51', 'C4A52', 'C4A59', 'C4A60', 'C4A61', 'C4A62',
		'C4A70', 'C4A71', 'C4A72', 'C4A8', 'C4A9', 'C50011', 'C50012', 'C50019', 'C50021', 'C50022', 'C50029', 'C50111', 'C50112', 'C50119', 'C50121', 'C50122',
		'C50129', 'C50211', 'C50212', 'C50219', 'C50221', 'C50222', 'C50229', 'C50311', 'C50312', 'C50319', 'C50321', 'C50322', 'C50329', 'C50411', 'C50412', 
		'C50419', 'C50421', 'C50422', 'C50429', 'C50511', 'C50512', 'C50519', 'C50521', 'C50522', 'C50529', 'C50611', 'C50612', 'C50619', 'C50621', 'C50622', 
		'C50629', 'C50811', 'C50812', 'C50819', 'C50821', 'C50822', 'C50829', 'C50911', 'C50912', 'C50919', 'C50921', 'C50922', 'C50929', 'C510', 'C511', 'C512', 
...
proc sql; 
   create table out.pe_12 as 
   select enrolid from out.pe_12
   group by enrolid; 
quit; 
...
data out.new_12; 
   if _N_=1 then do;
   declare hash ids(dataset:"out.pe_12");
...
* Collapse to enrollee level; 
proc sql; 
   create table out.new_12 as 
   select enrolid, age, min(year) as dx_year, min(svcdate) as dx_date, max(ip) as had_hosp from out.new_12
   group by enrolid; 
quit; 
...
data out.check_12;
   if _N_=1 then do;
   declare hash ids(dataset:"out.new_12");
   ids.definekey('enrolid');
   ids.definedone();
   end;
...
proc sql; 
   create table out.check_12 as 
   select enrolid, min(year) as fyear from out.check_12
   group by enrolid; 
quit;

data out.new_12; 
   merge out.new_12 out.check_12; 
   by enrolid;

   if dx_year - fyear < 1 then delete; 
run; 

proc delete data=out.check_12; 
run; 
...
* Collapse to enrollee level; 
proc sql;
   create table out.tomerge_12 as 
   select enrolid, max(year) as lyear from out.tomerge_12
   group by enrolid; 
quit; 

* Merge in with out.new_12, keep those with 5 years enrollment;
data out.new_12; 
   merge out.new_12 out.tomerge_12; 
...
proc sql; 
   create table out.fams_12 as
   select famid from out.new_12
   group by famid; 
quit; 
...
data out.subsequent_12; 
   if _N_=1 then do;
   declare hash ids(dataset:"out.fams_12");
   ids.definekey('famid');
   ids.definedone();
   end; 
...
data out.subsequent_12; 
   ...
   set out.subsequent_12; 
   if ids.find()^=0;  
run; 

* Collapse to enrollee level; 
proc sql; 
   create table out.subsequent_12 as 
   select enrolid, famid, age, min(year) as dx_year, min(svcdate) as dx_date, max(ip) as had_hosp from out.subsequent_12
   group by enrolid; 
quit; 
...
proc delete data=out.tomerge_12; 
run; 

proc delete data=out.fams_12; 
run; 
