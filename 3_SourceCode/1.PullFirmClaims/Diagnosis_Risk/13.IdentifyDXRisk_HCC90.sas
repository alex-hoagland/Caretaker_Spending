/*
*========================================================================*
* Program:   Identifying family risk from diagnosis (HCC_90)             *
*                                                                        *
...
/*------------------------------------------------------------------------*
 * 		ORDER OF OPERATIONS					  *
 * 0. Identify all HCC_90 claims in 2006				  *
 * 1. Identify all *new* HCC_90 claims in (2007-2010)			  *
...
proc sql; 
   create table out.pe_90 as 
   select enrolid from out.pe_90
   group by enrolid; 
quit; 
...
data out.new_90; 
   if _N_=1 then do;
   declare hash ids(dataset:"out.pe_90");
...
* Collapse to enrollee level; 
proc sql; 
   create table out.new_90 as 
   select enrolid, age, min(year) as dx_year, min(svcdate) as dx_date, max(ip) as had_hosp from out.new_90
   group by enrolid; 
quit; 
...
data out.check_90;
   if _N_=1 then do;
   declare hash ids(dataset:"out.new_90");
   ids.definekey('enrolid');
   ids.definedone();
   end;
...
proc sql; 
   create table out.check_90 as 
   select enrolid, min(year) as fyear from out.check_90
   group by enrolid; 
quit;

data out.new_90; 
   merge out.new_90 out.check_90; 
   by enrolid;

   if dx_year - fyear < 1 then delete; 
run; 

proc delete data=out.check_90; 
run; 
...
* Collapse to enrollee level; 
proc sql;
   create table out.tomerge_90 as 
   select enrolid, max(year) as lyear from out.tomerge_90
   group by enrolid; 
quit; 

* Merge in with out.new_90, keep those with 5 years enrollment;
data out.new_90; 
   merge out.new_90 out.tomerge_90; 
...
proc sql; 
   create table out.fams_90 as
   select famid from out.new_90
   group by famid; 
quit; 
...
data out.subsequent_90; 
   if _N_=1 then do;
   declare hash ids(dataset:"out.fams_90");
   ids.definekey('famid');
   ids.definedone();
   end; 
...
data out.subsequent_90; 
   ...
   set out.subsequent_90; 
   if ids.find()^=0;  
run; 

* Collapse to enrollee level; 
proc sql; 
   create table out.subsequent_90 as 
   select enrolid, famid, age, min(year) as dx_year, min(svcdate) as dx_date, max(ip) as had_hosp from out.subsequent_90
   group by enrolid; 
quit; 
...
proc delete data=out.tomerge_90; 
run; 

proc delete data=out.fams_90; 
run; 
