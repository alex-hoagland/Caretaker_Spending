...
data out.maintenance_costs_hcc48_op; 
   if _N_=1 then do;
...
proc sql; 
   create table out.maintenance_costs_hcc48_op as 
   select enrolid, hcc, year, sum(oop) as op_oop, sum(pay) as op_pay from out.maintenance_costs_hcc48_op
...
data out.maintenance_costs_hcc48_op; 
   set out.maintenance_costs_hcc48_op;
