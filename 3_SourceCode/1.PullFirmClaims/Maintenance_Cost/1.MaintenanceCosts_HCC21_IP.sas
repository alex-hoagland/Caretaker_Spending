...
data out.maintenance_costs_hcc21_ip; 
   if _N_=1 then do;
   declare hash ids(dataset:"out.pe_maintenancesample_hcc21");
...
proc sql; 
   create table out.maintenance_costs_hcc21_ip as 
   select enrolid, hcc, year, sum(oop) as ip_oop, sum(pay) as ip_pay from out.maintenance_costs_hcc21_ip
...
data out.maintenance_costs_hcc21_ip; 
   set out.maintenance_costs_hcc21_ip;
