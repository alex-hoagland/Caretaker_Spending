...
/*------------------------------------------------------------------------*
 * 		ORDER OF OPERATIONS					  *
 * 0. Identify all with HCC_12 claims in 2015-2018			  *
 * 1. Pull enrollment files for all families in (0.)			  *
 * 2. The rest is done in stata						  *
 *------------------------------------------------------------------------*/;


/* --- 0. All Diabetics, 2015 - 2018 -----------------------------------------*/; 
data out.allclaims_hcc12_2015; 
...
proc sql; 
   create table out.allenrollees_hcc12_2015 as 
   select enrolid from out.allclaims_hcc12_2015
   group by enrolid; 
quit; 

data out.allenrollees_hcc12_2015;
   set out.allenrollees_hcc12_2015;

   famid = floor(enrolid/100); 
run; 

/* --- 1. Enrollment Info for all involved families -----------------------------------------*/; 
data out.allenrollment_12_2015; 
   if _N_=1 then do;
   declare hash ids(dataset:"out.allenrollees_hcc12_2015");
   ids.definekey('famid');
   ids.definedone();
   end;
